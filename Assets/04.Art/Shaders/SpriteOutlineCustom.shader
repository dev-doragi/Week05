Shader "Sprites/OutlineCustom"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{
				// 월드 스케일 값을 가져옵니다. (X, Y 방향의 길이를 추출)
				...
				// 두께를 스케일값으로 나누어 보정합니다.
				float thicknessX = (_Thickness / _MainTex_TexelSize.z) / worldScale.x;
				float thicknessY = (_Thickness / _MainTex_TexelSize.w) / worldScale.y;
				int steps = 100;
